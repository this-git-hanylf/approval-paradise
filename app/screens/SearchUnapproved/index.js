...
const SearchUnapproved = props => {
  const [popular, setPopular] = useState(HomePopularData);
  const {colors} = useTheme();
  const [loading, setLoading] = useState(true);
  const user = useSelector(state => state.user);
  const getData = useSelector(state => state.data_request);
  ...
  const getDataApproval = useCallback(() => dispatch(getdata(statusPending)), [
    statusPending,
    dispatch,
  ]);
...
export default SearchUnapproved;
